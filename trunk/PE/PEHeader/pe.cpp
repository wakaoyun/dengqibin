...
void Cpe::CalcAddress(const void *base)
{
	...
	if(IsBadReadPtr(header,sizeof(*header)))
	{
		...
		cout<<"No PE header, probable DOS executable."<<endl;
		return;
	}
...
{	
	...
	if(dwSpace<50)
	{
		cout<<"No room to write the data!"<<endl;
...
BOOL Cpe::WriteNewEntry(int ret, long offset, DWORD dwAddress)
{
	...
	if(-1==retf)
	{
		cout<<"Error seek in WriteNewEntry."<<endl;
